...
def naturaldelta(
	...
	Args:
		...
		months (bool): If `True`, then a number of months (based on 30.5 days) will be used for fuzziness between years.
		minimum_unit (str): The lowest unit that can be used.

	Returns:
		str (str or `value`): A natural representation of the amount of time elapsed unless `value` is not datetime.timedelta or cannot be converted to int (cannot be float due to 'inf' or 'nan'). In that case, a `value` is returned unchanged.
	...
	if not years and days < 1:
		...
		if use_months:
			...
			return (
				_ngettext("1 year, %d month", "1 year, %d months", num_months) % num_months
			)
...
def precisetime(value: dt.datetime | dt.timedelta | float, minimum_unit: str = "seconds", suppress: list[str] = (), format: str = "%d", when: dt.datetime | None = None) -> str:
	"""Return a precise representation of a time relative to now, with tense."""
	...
	# Determine tense
	future = date > now
	delta_str = precisedelta(delta, minimum_unit=minimum_unit, suppress=suppress, format=format)
	ago = _("%s from now") if future else _("%s ago")
	
	if delta_str == "a moment":
		return "now"

	return str(ago % delta_str)
